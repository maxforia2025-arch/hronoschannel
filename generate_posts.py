#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""⏳ АВТОПОПОЛНЕНИЕ БАНКА канала «Хронос» через Claude API.
...
def build_prompt(cfg, existing, n):
    ...
    pillars = cfg.get("pillars_hint", "Один день, Развилка, Тихий факт, Карта")
    return (
        "Ты — редактор Telegram-канала «Хронос».\n"
        "Ниша канала: Один день, который перевернул историю — короткие реконструкции переломных суток\n"
        "Тон: сдержанный, документальный, с нарастающим напряжением\n"
        ...
        "5. ДАТЫ ВСЕГДА ЦИФРАМИ: «9 ноября 1989 года», а не «девятое ноября».\n"
        "   День и год — цифрами, месяц — словом. Время — тоже цифрами: «01:23».\n"
        "6. Редстандарт канала: только документированные события и точные даты.\n"
        "   Никаких псевдоисторических баек. Спорную деталь либо не бери, либо прямо\n"
        "   назови спорной. Числа давай диапазоном, если источники расходятся.\n\n"
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Автопополнение банка «Хронос»")
