#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""⏳ ПОСТЕР канала «Хронос» — берёт следующий пост из банка и шлёт в Telegram.
...
который лежит в .gitignore). В коде секретов нет и быть не должно.
    BOT_TOKEN=123:ABC   CHANNEL_ID=@hronos_daily   [FUNNEL_BOT_USERNAME=...]
"""
...
def log(msg):
    print("[hronos] " + str(msg), flush=True)
...
def format_post(post, cfg):
    ...
    name = html.escape(str(cfg.get("channel_name", "Хронос")))
    handle = html.escape(str(cfg.get("channel_handle", "@hronos_daily")))
    ...
    if funnel:
        link = "https://t.me/" + funnel + "?start=" + str(cfg.get("src", "hrn")) + "_tg"
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Постер канала «Хронос»")
